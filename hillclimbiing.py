    ...
    while evaluations_count < MEB:
        for next_city in mutation_operator(best_path):
            if evaluations_count == MEB:
                break
            str1 = ''.join(str(e) for e in next_city)  # Skip calculating the cost of repeated paths
            if str1 in dict:
                evaluations_count += 1
                continue
            next_tCost = tour_cost(next_city, cost_func)  # Store it in the dictionary
            dict[str1] = next_tCost
            evaluations_count += 1
            # Selecting the path with the lowest cost
...
def simulated_annealing(no_cities, cost_func, MEB, seed1):
    ...
    for temperature in temp_schedule:
        flag = False
        # Examining moves around our current path
...
while keeprunning:
    ...
    print("The cost of the best solution:", best_cost, file=open("2runs.txt", "a"))
    print("The Path of the best solution:", best_path, file=open("2runs.txt", "a"))
    print("The Value of MEB count:", num_evaluations, file=open("2runs.txt", "a"))
    print("Time taken: %.2f seconds" % (time.time() - start_time), file=open("2runs.txt", "a"))
